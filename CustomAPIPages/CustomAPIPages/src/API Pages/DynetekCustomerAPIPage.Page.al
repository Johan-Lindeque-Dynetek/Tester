page 50000 "Dynetek Customer API Page"
{
    APIGroup = 'DynetekApi';
    APIPublisher = 'Dynetek';
    APIVersion = 'v1.0';
    Caption = 'dynetekCustomerAPIPage';
    DelayedInsert = true;
    EntityName = 'DynetekCustomer';
    EntitySetName = 'DynetekCustomer';
    PageType = API;
    ...
    layout
    {
        area(Content)
        {
            repeater(General)
            {
                ...
                // -> MPS-JL
                field(dynCustomFields; Rec."DYN Custom Fields")
                {
                    Caption = 'DYN Custom Fields';
                }
                // field(dynCustomFields; GetCustomFields())
                // {
                //     Caption = 'DYN Custom Fields';
                // }
                field(dynLastModified; Rec."DYN Last Modified")
                {
                    Caption = 'DYN Last Modified';
                }
                field(customFieldsB64; GetCustomFields())
                {
                    Caption = 'Custom Fields as B64';
                }
                // <-
            }
        }
    ...
    begin
        ...
        // DynamicFieldReturnHandler.BuildCustomerFieldsListFromReqList(CustomField);

        // RecToJsonHelper.Init();
        // RecToJsonHelper.Append(Rec.RecordId());
        // //TODO: Append custom fields 1 by 1
    ...
    trigger OnOpenPage();
    begin
        CustomField := Rec.GetFilter("DYN Custom Fields");
        CustomField := CopyStr(CustomField, 2, StrLen(CustomField) - 2);
        Rec.SetRange("DYN Custom Fields");


    end;

    local procedure SkryfToetsTbl()
    var
        toets: Record toets;
    begin
        toets.Init();
        toets."custom fields" := CustomField;
        toets."Time recieved" := CurrentDateTime;
        toets.RecId := 'RecID: ' + Format(Rec.RecordId.TableNo);
        toets.Insert();
    end;

    local procedure GetCustomFields(): Text
    begin
        // // DynamicFieldReturnHandler.BuildCustomerFieldsListFromReqList(CustomField);
        ...
        // // exit('"CustomFields": ' + RecToJsonHelper.OutputStrJSONObject());
        // exit(Format(DynamicFieldReturnHandler.GetCustomFieldsList()));


        RecToJsonHelper.Init();
        RecToJsonHelper.BuildCustomerFieldsListFromReqList(CustomField);
        RecToJsonHelper.Append(Rec.RecordId());
        RecToJsonHelper.Insert();
